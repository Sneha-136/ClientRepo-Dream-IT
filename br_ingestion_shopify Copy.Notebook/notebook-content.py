...
# def get_date_chunks1(object, min_date, max_date, shopify_store, access_token):
...
#         start_date = start_date + relativedelta(months=1) - timedelta(days=2)
...
#     print(f"Starting fetch for store: {store}, object: {object}, range: {min_timestamp} to {max_timestamp}")
...
#         "order": "id asc",
#         # "created_at_min": min_timestamp,
#         # "created_at_max": max_timestamp,
...
#         except requests.exceptions.RequestException as e:
#             print(f"Request failed on attempt {attempt} for store {store}: {e}")
#             break

#         try:
#             batch = response.json().get(object, [])

#         except ValueError as e:
#             print(f"Failed to parse JSON response for store {store}: {e}")
#             break

#         if not batch:
#             print(f"No more records found for store {store} in this chunk.")
#             break
...
#             if "since_id" in batch.keys():
#                 since_id = batch.get('since_id')
#             else:
#                 since_id = None
                
#         # print(f"Fetched {len(batch)} records (total so far: {len(all_records)}) for store {store}")
#         time.sleep(0.25)

#     # print(f"Completed fetch for store {store}, total records: {len(all_records)}")
#     return all_records
