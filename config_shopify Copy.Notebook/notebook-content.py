...
# input_config = '''{
#        "stores": {
#            "company-uk.myshopify.com": {
#               "access_token": "shpat_9f80a2958",
#                "prefix": "_mvuk",
#                "sources": {
#                    "orders": {"active_flag": true},
#                    "products": {"active_flag": true}
#                }
#            },
#            "company-us.myshopify.com": {
#                "access_token": "shpat_9f66644558",
#                "prefix": "_mvus",
#                "sources": {
#                    "orders": {"active_flag": false},
#                    "products": {"active_flag": false}
#                }
#            }
#        }
#    }
...
# print("Staging Lakehouse Path:", STAGING_LAKEHOUSE_PATH)
# print("Bronze Lakehouse Path:", BRONZE_LAKEHOUSE_PATH)
...
# 
# 
# columns are store, access token, table, source, prefix, last sync
...
# with open("./builtin/fernet.key", "wb") as builtin_file:
#     builtin_file.write(private_key)

# private_key = private_key.decode("utf-8")

# key_df = spark.createDataFrame([private_key], "string")
# key_df.write.mode("overwrite").text(f"{BRONZE_LAKEHOUSE_PATH}/Files/fernet_key.txt")
...
# key_df = spark.read.text(f"{BRONZE_LAKEHOUSE_PATH}/Files/fernet_key.txt")
# private_key = key_df.collect()[0][0].encode("utf-8")
# fernet = Fernet(private_key)
...
#     source_to_table_mapping[source_name] = f"sil.shopify.{source_name}"
...
# # Build dynamic when conditions for table mapping
# table_mapping_expr = None
# for source_name, table_path in source_to_table_mapping.items():
#     condition = col("source").contains(source_name)
#     if table_mapping_expr is None:
#         table_mapping_expr = when(condition, lit(table_path))
#     else:
#         table_mapping_expr = table_mapping_expr.when(condition, lit(table_path))

# # Apply default if no match
# table_mapping_expr = table_mapping_expr.otherwise(lit(None))

# new_processing_rows = new_processing_rows.withColumn("table", table_mapping_expr)
