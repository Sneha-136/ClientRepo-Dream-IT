...
def validate_multi_tenant_setup(token):
    ...
    print("\n" + "=" * 60)
    print("🔍 VALIDATING MULTI-TENANT SETUP")
    print("=" * 60)
    
    # Test 1: Can we access Fabric API in target tenant?
    print("\n1️⃣  Testing Fabric API access...")
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   ✅ Success! Found {len(capacities)} capacity/capacities")
        elif response.status_code == 403:
            print("   ❌ Access Denied!")
            print("   → Check: Service principal enabled in Software tenant's Fabric settings")
            return False
        elif response.status_code == 401:
            print("   ❌ Authentication Failed!")
            print("   → Check: Admin consent granted in Software tenant")
            return False
        else:
    ...
    if CAPACITY_ID:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   ✅ Capacity accessible: {cap_data.get('displayName')}")
            elif response.status_code == 403:
                print("   ❌ Cannot access capacity!")
                print("   → Check: Service principal added to capacity permissions")
                return False
            elif response.status_code == 404:
                print("   ❌ Capacity not found!")
                print("   → Check: CAPACITY_ID is correct for Software tenant")
                return False
            else:
...
def create_workspace(token, workspace_name, capacity_id):
    ...
    print("\n" + "=" * 60)
    print("🏗️  CREATING NEW WORKSPACE IN SOFTWARE TENANT")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n✅ Workspace created successfully in Software tenant!")
...
def add_workspace_admins(admin_users_string, workspace_id):
    ...
    print("\n" + "=" * 60)
    print("👥 ADDING WORKSPACE ADMINISTRATORS (SOFTWARE TENANT)")
    print("=" * 60)
    ...
    if not admin_users_string or not admin_users_string.strip():
        ...
        print("💡 Set ADMIN_USERS='email1@software.com,email2@software.com' to add admins")
...
def main():
    print("=" * 60)
    print("🚀 MULTI-TENANT Fabric Workspace Git Sync Workflow")
    print("=" * 60)
    print(f"🏢 Service Principal Tenant (Dream IT): {TENANT_ID}")
    print(f"🎯 Target Tenant (Software): {TARGET_TENANT_ID}")
    print(f"📦 Repository: {GITHUB_OWNER}/{REPO_NAME}")
    print(f"🌿 Branch: {BRANCH_NAME}")
    ...
    try:
        ...
        if not validate_multi_tenant_setup(token):
            print("\n❌ Multi-tenant setup validation failed!")
        ...
        # Final summary
        print("\n" + "=" * 60)
        print("🎉 MULTI-TENANT WORKFLOW COMPLETED SUCCESSFULLY!")
        print("=" * 60)
        print(f"✅ Service Principal from Dream IT tenant")
        print(f"✅ Created/managed workspace in Software tenant")
        if create_new:
